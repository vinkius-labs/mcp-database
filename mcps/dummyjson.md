...
[![Deploy on Vinkius Edge](https://img.shields.io/badge/Deploy%20on-Vinkius%20Edge-blue?style=for-the-badge)](https://vinkius.com/en/ai-agent-connect/dummyjson)
...
**Q: How can I retrieve a list of all products with a specific limit?**
You can use the `list_products` tool and provide the `limit` parameter to control the number of results returned by the API.

**Q: Is it possible to simulate adding a product to the database?**
...
**Q: How do I check the carts belonging to a specific user?**
...
1. View installation instructions and explore the server: [https://vinkius.com/en/ai-agent-connect/dummyjson](https://vinkius.com/en/ai-agent-connect/dummyjson)
...
5. Click the **+** button in any chat and enable **DummyJSON** under Connectors.
...
3. Set Type to "SSE" (or "streamable HTTP"), enter `dummyjson` as the name, and paste the MCP server link (`https://edge.vinkius.com/[TOKEN]/mcp`).
4. Click **Save** — Cursor will connect and list all **DummyJSON** tools.
