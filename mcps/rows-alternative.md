# Rows Alternative MCP Server

[![Deploy on Vinkius Edge](https://img.shields.io/badge/Deploy%20on-Vinkius%20Edge-blue?style=for-the-badge)](https://vinkius.com/en/ai-agent-connect/rows-alternative)
...
## Available Tools (11)
- **delete_rows_spreadsheet**: Delete a spreadsheet
- **get_rows_folder**: Get details for a specific folder
- **get_rows_range_cells**: Get detailed cell objects
- **get_rows_spreadsheet_details**: Get metadata for a spreadsheet
- **list_rows_spreadsheets**: List your Rows spreadsheets
- **append_rows_values_to_table**: Add new rows to a table
- **create_rows_spreadsheet**: Create a new spreadsheet
- **get_rows_range_values**: g., A1:B5).

Get values from a specific range
- **get_rows_workspace_info**: Get Rows workspace details
- **list_rows_folders**: List workspace folders
- **update_rows_range_values**: Overwrite values in a range


## 💬 Prompt Examples

Here are some examples of how you can interact with the **Rows Alternative** MCP server using an AI Agent (Claude, ChatGPT, etc.).
...
1. View installation instructions and explore the server: [https://vinkius.com/en/ai-agent-connect/rows-alternative](https://vinkius.com/en/ai-agent-connect/rows-alternative)
...
5. Click the **+** button in any chat and enable **Rows Alternative** under Connectors.
...
3. Set Type to "SSE" (or "streamable HTTP"), enter `rows-alternative` as the name, and paste the MCP server link (`https://edge.vinkius.com/[TOKEN]/mcp`).
4. Click **Save** — Cursor will connect and list all **Rows Alternative** tools.
