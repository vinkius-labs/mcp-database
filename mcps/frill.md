...
[![Deploy on Vinkius Edge](https://img.shields.io/badge/Deploy%20on-Vinkius%20Edge-blue?style=for-the-badge)](https://vinkius.com/en/ai-agent-connect/frill)
...
**Q: Can I create a new product idea directly through the AI?**
Yes! Use the `create_idea` tool by providing a name and description. You can also optionally associate it with specific topics using their IDXs.

**Q: How do I retrieve the latest product announcements?**
You can use the `list_announcements` tool. It supports pagination via the `after` parameter and allows you to set a `limit` for the number of results returned.

**Q: Is it possible to manage announcement categories?**
Absolutely. You can use `list_categories`, `create_category`, `update_category`, and `delete_category` to fully manage how your announcements are organized.
...
1. View installation instructions and explore the server: [https://vinkius.com/en/ai-agent-connect/frill](https://vinkius.com/en/ai-agent-connect/frill)
...
5. Click the **+** button in any chat and enable **Frill** under Connectors.
...
3. Set Type to "SSE" (or "streamable HTTP"), enter `frill` as the name, and paste the MCP server link (`https://edge.vinkius.com/[TOKEN]/mcp`).
4. Click **Save** — Cursor will connect and list all **Frill** tools.
