# COR Alternative MCP Server

[![Deploy on Vinkius Edge](https://img.shields.io/badge/Deploy%20on-Vinkius%20Edge-blue?style=for-the-badge)](https://vinkius.com/en/ai-agent-connect/cor-alternative)
...
Here are some examples of how you can interact with the **COR Alternative** MCP server using an AI Agent (Claude, ChatGPT, etc.).
...
Yes! The `get_cor_workspace_project` tool allows your agent to retrieve high-fidelity profitability metrics and financial health data for any specific project.

**Q: How do I list my organization's teams?**
Use the `list_cor_workspace_teams` tool to retrieve the complete high-fidelity directory of teams along with their unique identifiers for precise orchestration.
...
1. View installation instructions and explore the server: [https://vinkius.com/en/ai-agent-connect/cor-alternative](https://vinkius.com/en/ai-agent-connect/cor-alternative)
...
5. Click the **+** button in any chat and enable **COR Alternative** under Connectors.
...
3. Set Type to "SSE" (or "streamable HTTP"), enter `cor-alternative` as the name, and paste the MCP server link (`https://edge.vinkius.com/[TOKEN]/mcp`).
4. Click **Save** — Cursor will connect and list all **COR Alternative** tools.
